...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            ...
            attribute = self.all_configurations[self.t,i]
    ...
    def post_step(self, world):
        ...
        if self.tt > 4: # minimal interval to change agents
            self.tt = 0
            if True:
                if self.num_agents == 5:
                    add = (np.random.rand() > 0.5)
                elif self.num_agents == 4:
                    add = True
                else:
                    add = False
                if add: # add 1 more agent
                    for i, agent in enumerate(world.agents):
                        if not agent.alive:
                            agent.alive = True
                            th = np.random.rand() * 2 * np.pi
                            agent.state.p_pos = np.array([np.cos(th), np.sin(th)]) * 0.12
                            agent.state.p_vel = np.zeros((2,))
                            agent.at = 0
                            agent.movable = True
...
def make_env_hard(benchmark=False, discrete_action=True):
    '''
